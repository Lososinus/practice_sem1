...
def drawgas(screen, n, x, y, r, color, width, height):
	# отрисовка частиц
	for i in range(n):
		circle(screen, color[i], (x[i], y[i]), r[i])

def drawgas_blurred(screen, n, x, y, r, color, width, height):
	# отрисовка следа частиц
	for i in range(n):
		tmp_surf = pygame.Surface((2*r[i], 2*r[i]))
		circle(tmp_surf, color[i], (r[i], r[i]), r[i])
		tmp_surf.set_colorkey((0, 0, 0))
		tmp_surf.set_alpha(3)
		screen.blit(tmp_surf, (x[i]-r[i], y[i]-r[i]))
...
fps = 0
iter=0
gas = pygame.Surface((width, height))
gas.set_alpha(250)
dt = 0
while not finished:
	subclock.tick()
	...
	calc_iter(n, x, y, vx, vy, dt*f_forward, g)
	for i in range(n):
	...
	drawgas_blurred(gas, n, x, y, r, color, width, height)
	screen.blit(gas, (0,0))
	drawgas(screen, n, x, y, r, color, width, height)
	dt_draw = subclock.tick()
	
	font.render_to(screen, (10, 1810), "fps:      {0:8.2f} iter: {1:5d}".format(fps, iter), (232, 98, 129), (0, 0, 0))
	font.render_to(screen, (10, 1870), "physics time: {0:4d}".format(dt_compute), (232, 98, 129), (0, 0, 0))
	font.render_to(screen, (10, 1930), "drawgas time: {0:4d}".format(dt_draw), (232, 98, 129), (0, 0, 0))
	font.render_to(screen, (10, 1990), "write time:   {0:4d}".format(dt_write), (232, 98, 129), (0, 0, 0))
	font.render_to(screen, (10, 2050), "total time:   {0:4d}".format(delta_t), (232, 98, 129), (0, 0, 0))
	draw_chart(screen, 610, 1870, 1050, 2090, vx_stat, st_size*2+1)
	dt_write = subclock.tick()
	
	#line(screen, (200, 200, 200), (width/2-wx*400, height/2-wy*400), (width/2+wx*400, height/2+wy*400))
